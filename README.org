...
1. Resolve symbol. If ~SYMBOL~ (string) is non-nil, just use it. If it is nil, you need to either guess for one or prompt the user for the symbol. ~PROMPT~ represents the prompting strategy, there are three strategies:
...
You can guess the symbol by ~(symbol-name (symbol-at-point))~ and prompt for a symbol by ~completing-read~. In addition, ghelp provides a helpful macro ~ghelp-maybe-prompt~ to handle ~PROMPT~ for you (as shown in the example).
